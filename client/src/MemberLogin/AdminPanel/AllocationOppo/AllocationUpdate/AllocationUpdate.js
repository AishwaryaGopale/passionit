...
const AllocationUpdate = () => {
  ...
  const GetMember = (id) => {
    axios({
      url:(API.GET_OPPORTUNITYALLOCATION_API(id)),
      method: 'GET',
      headers: {
  ...
  const AddData = () => {
    ...
    var id = ids;
    axios({ 
      url: (API.UPDATE_OPPORTUNITYALLOCATION_API(id)),
      method: 'PUT',
      data: payload,
      contentType: 'application/json',
    }).then((res) => {
                  ...
                  <div className="grid grid-cols-1 gap-4 text-sm gap-y-2 md:grid-cols-5">
                    ...
                    <div className="text-center md:col-span-5">
                      <div className="inline-flex items-end">
                        <button className="px-4 py-2 font-bold text-white bg-red-500 rounded hover:bg-red-700" onClick={() => AddData()}>
                          Submit
                        </button>
                      </div>
                    </div>

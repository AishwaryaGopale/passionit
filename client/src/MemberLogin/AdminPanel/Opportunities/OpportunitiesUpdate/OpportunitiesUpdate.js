...
const [idx,setId]=useState([])
  ...
  const GetMember = (id) => {
    axios({
      url: (API.GET_OPPORTUNITY_API(id)),
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
    }).then((res) => {
      opportunity_type_id.current.value = res.data.opportunity_type_id;
      ...
      opportunity_provider.current.value = res.data.projectn;
    }).catch((error) => {
  ...
  const AddData = async () => {
    const playload = {
      opportunity_type: opportunity_type_id.current.value,
    ...
    var id=idx;
    axios({
      url: (API.PUT_OPPORTUNITY_API(id)),
      method: 'put',
      data: playload,
      headers: {
        'Content-Type': 'application/json',
                ...
                <div class="lg:col-span-2">
                  <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-4">
                    <div class="md:col-span-2">
                      ...
                      <input type="text" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" ref={opportunity_type_id} />
                    </div>
                    ...
                    <div class="md:col-span-2">
                      <label for="city">Project Name</label>
                      <input type="text" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" ref={projectname} />
                    </div>
                    ...
                    <div class="md:col-span-5 text-right">
                      <div class="inline-flex items-end">
                        <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" onClick={() => AddData()}>Submit</button>
                      </div>
                    </div>
                  </div>
                </div>
